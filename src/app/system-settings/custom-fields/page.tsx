...
export default function CustomFieldManagement(){
  ...
  const [fieldBelongs, setFieldBelongs] = useState<string>('');
  const [fieldType, setFieldType] = useState<string>('');
  const [fieldName, setFieldName] = useState<string>('');
  const [gridValue, setGridValue] = useState<string>('12');
  const [fieldValues, setFieldValues] = useState<string>('');
  const [isRequired, setIsRequired] = useState<boolean>(false);
  const [onTable, setOnTable] = useState<boolean>(false);

  const [expanded, setExpanded] = useState<ExpandState>({ student: false, staff: false });
  const [customFields, setCustomFields] = useState<CustomFieldsData>({ student: [], staff: [] });
  ...
  const handleSave = () => {
    ...
    setCustomFields((prev) => ({
      ...prev,
      [fieldBelongs]: [...prev[fieldBelongs as keyof CustomFieldsData], newField],
    }));
  ...
  return customFields ? (
    <div className="flex flex-col md:flex-row gap-4 p-4 md:p-6 bg-gray-100 min-h-screen">
      ...
      <div className="w-full md:w-1/2 bg-white p-4 md:p-6 rounded shadow-sm">
        ...
        <div className="space-y-4">
          {(['student', 'staff'] as (keyof CustomFieldsData)[]).map((type) => (

...
interface SettingDetails {
  ...
  is_duplicate_fees_invoice?: number;
  adm_auto_insert?: number;
  adm_prefix?: string;
  adm_start_from?: number;
  adm_no_digit?: number;
  staffid_auto_insert?: number;
  staffid_prefix?: string;
  staffid_start_from?: number;
  staffid_no_digit?: number;
  pan_number?: string;
  gst_no?: string;
  service_tax_no?: string;
  vat_no?: string;
  cin_no?: string;
  sch_dise_code?: string;
  app_primary_color_code?: string;
  app_secondary_color_code?: string;
  mobile_api_url?: string;
  mytradinglink?: string;
  my_question?: string;
  sch_soc_name?: string;
  sch_name_primary?: string;
  sch_name_secondary?: string;
  sch_recog_primary?: string;
  sch_recog_secondary?: string;
  sch_udise_primary?: string;
  sch_udise_secondary?: string;
  sch_city?: string;
  sch_state?: string;
  principal_sign?: string;
  clerk_sign?: string;
  examiner_sign?: string;
  sch_establish?: string;
  alt_phone?: string;
  alt_email?: string;
  sch_name_high_secondary?: string;
  sch_recog_high_secondary?: string;
  sch_udise_high_secondary?: string;
  certificate_print_lang?: number;
  biometric?: number;
...
interface UpdatePayload {
  id: number;
  sch_session_id: number;
  fee_due_days: number;
  sch_name: string;
  sch_phone: string;
  sch_start_month: string;
  sch_start_week: string;
  sch_address: string;
  sch_email: string;
  sch_lang_id: number;
  sch_currency_symbol: string;
  sch_timezone: string;
  sch_currency: string;
  currency_place: string;
  sch_date_format: string;
  sch_is_rtl: string;
  theme: string;
...
const GeneralSettings: React.FC = () => {
    ...
    const [activeLink, setActiveLink] = useState<ActiveLinkType>("general-setting");
    const [settingDetails, setSettingDetails] = useState<SettingDetails | undefined>();
    const [updatedData, setUpdatedData] = useState<SettingDetails | undefined>();
    ...
    useEffect(() => {
        const fetchSettings = async (): Promise<void> => {
            try {
                const data: SettingDetails = await getGeneralSettingDetails();
    ...
    const handleChanges = (field: keyof SettingDetails, e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement>): void => {
    ...
    const handleSubmit = async (): Promise<void> => {
        try {
            ...
            console.log(res);
            if ((res as { status?: number }).status === 200) notify();
        } catch (err) {
            console.log(err);
        }
    };
    ...
    return (
        <div className="mt-6 h-fit lg:flex sm:flex-row px-5 md:block intro-y ">
            ...
            <div className="lg:w-4/5 md:w-2/3 bg-white">
                {renderContent()}
            </div>
        </div>
    );

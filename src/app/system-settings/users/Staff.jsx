...
function Staff() {
  ...
  const toggleActive = (staffId) => {
    const updated = staff.map((member) =>
      member["Staff ID"] === staffId ? { ...member, Action: !member.Action } : member
    );
    setStaff(updated);
  };
  ...
  const handleDownloadClick = () => {
    const csv = [
      ["Staff ID", "Name", "Email", "Role", "Designation", "Department", "Phone", "Action"],
      ...filteredStaff.map(m => [
        m["Staff ID"],
        m.Name,
        m.Email,
            ...
            <table className="min-w-full divide-y divide-gray-200">
              ...
              <tbody className="bg-white divide-y divide-gray-100">
                ...
                {filteredStaff.length === 0 && (
                  <tr>
                    <td colSpan={8} className="text-center px-4 py-3 text-sm text-gray-500">

...
import staffData from './StaffData'; // Assuming you have the staff data in this file
...
function Staff() {
  ...
  const handlePrintClick = () => {
    const content = document.getElementById('staff-table').innerHTML;
              ...
              <td className="px-4 py-3 text-right">
                ...
                >
                  <div className="relative w-12 h-6">
                    {/* Background track */}
                    <div 
                      className={`absolute inset-0 rounded-full transition-colors duration-300 ${item.action ? 'bg-green-500' : 'bg-gray-200'}`}
                    />
                    {/* Toggle circle */}
                    <div 
                      className={`absolute top-1 w-4 h-4 bg-white rounded-full shadow transition-all duration-300 ${item.action ? 'left-7' : 'left-1'}`}
                    />
                  </div>
                </button>
              </td>

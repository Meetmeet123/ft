...
interface StudentItem {
  ...
  Action?: boolean; // If you need this toggle flag
}
...
function Student() {
  ...
  interface ModuleResponse {
    data: {
      studentPermissionList: StudentItem[];
    };
  }
  ...
  const toggleSwitch = (name: string, key: keyof Omit<StudentItem, 'id' | 'name' | 'short_code' | 'group_id' | 'Action'>) => {
    const updated = students.map((item) =>
    ...
    <div className="overflow-x-auto">
      <div className="flex flex-wrap items-center justify-between gap-4 border-b p-4">
        ...
        <div className="flex flex-wrap items-center gap-4">
          <button onClick={handleExcelExport} className="text-gray-500" title="Export Excel">
            <Database size={18} />
          </button>
        ...
        <table className="min-w-full divide-y divide-gray-200">
          ...
          <tbody className="bg-white divide-y divide-gray-200">
            {filteredStudents.map((item) => (
              <tr key={item.name}>
                <td className="px-4 py-3 text-sm text-blue-600">{item.id}</td>
                <td className="px-4 py-3 text-sm text-blue-600">{item.name}</td>
                <td className="px-4 py-3 text-sm text-blue-600">{item.short_code}</td>
                ...
                <td className="px-4 py-3 text-sm text-blue-600">{item.group_id}</td>
              </tr>
            ))}

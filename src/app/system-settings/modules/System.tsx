...
interface Student {
  ...
  system: 0 | 1;
  student: 0 | 1;
  [key: string]: any; // for any extra keys like 'Action'
}
...
function System() {
  ...
  const toggleSwitch = (name: string, key: keyof Student) => {
  ...
  const handleDownloadClick = () => {
    ...
    XLSX.writeFile(workbook, 'system_data.xlsx');
  };

  useEffect(() => {
    const fetchData = async () => {
      try {
        const data = await getModuleData() as ApiResponse;
        setStudents(data.data.permissionList);
      } catch (err) {
        console.log(err);
      }
    };
      ...
      <div id="print-section" className="overflow-x-auto">
        <table className="min-w-full divide-y divide-gray-200">
          <thead className="bg-gray-50">
            <tr>
              {["ID", "Name", "Short Code", "Active", "System"].map((title, idx) => (
                <th
                  key={idx}
          ...
          <tbody className="bg-white divide-y divide-gray-200">
            ...
            {filteredStudents.length === 0 && (
              <tr>
                <td colSpan={5} className="text-center px-4 py-3 text-sm text-gray-500">

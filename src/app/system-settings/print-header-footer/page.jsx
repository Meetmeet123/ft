...
const PrintHeaderFooter = () => {
  ...
  const [headerImageFile, setHeaderImageFile] = useState(null); // New state for actual file
  ...
  const handleSave = async() => {
    // Check if we have a header image file to upload or existing image
    const hasNewFile = headerImageFile !== null;
    const hasExistingImage = headerFooterData.length !== 0 && headerFooterData[activeIndex].header_image;
    
    if (!hasNewFile && !hasExistingImage) {
      alert('Please upload a header image before saving.');
      return;
    }
    ...
    try {
      // Create FormData like in the signature component
      const formData = new FormData();
      formData.append('print_type', headerFooterData[activeIndex].print_type);
      formData.append('footer_content', footerContent);
      ...
      // If there's an existing image but no new file, you might need to handle this differently
      // depending on your API requirements
      if (!hasNewFile && hasExistingImage) {
        // You might need to pass the existing image URL or handle it server-side
        formData.append('existing_header_image', headerFooterData[activeIndex].header_image);
      }
      ...
      if (res.success) {
        alert('Header and footer settings saved successfully!');
        
        // Refresh the data to get updated information
        const updatedData = await getHeaderFooterDetails();
        setHeaderFooterData(updatedData.result);
        
        // Reset the file states
        setHeaderImageFile(null);
        // Keep the headerImage state to show the uploaded image
        ...
        >
          {headerFooterData.length !== 0 && (headerImage || headerFooterData[activeIndex].header_image) ? (
            <div className="relative">
              <img
                src={headerImage || headerFooterData[activeIndex].header_image}
                alt="Header"

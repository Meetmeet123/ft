...
export default function Session() {
  ...
  if (sessionName.trim()) {
    ...
    if (editMode && editId) {
      ...
      const res = await updateSessionDetails(newData);
      fetchData();
      if(res.status === 200) toast.success("New Session Added");
      console.log(res)
  ...
  const handleEdit = (session: EditSession): void => {
    setEditMode(true);
  ...
  const handleDelete: DeleteSessionHandler = async (id) => {
    try{
      const res = await deleteSessionDetails(id) as { status?: number };
      if(res?.status === 200) {
        toast.success("Session Deleted")
        setSessions(sessions.filter((session: SessionType) => session.id !== id));
      }
    }catch(err){
      console.log(err)
      toast.error("Session cannot be deleted")
    }
  };
  ...
  interface ToggleSessionActive {
    (id: number): void;
  }

  const toggleSessionActive: ToggleSessionActive = (id) => {
    setSessions(sessions.map((session: SessionType) => 
      session.id === id 

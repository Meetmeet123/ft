...
export default function StudentFees() {
  ...
  const handleExportExcel=()=>{
    ...
    studentInfo.forEach(row => {
      table += '<tr>';
      headers.forEach(header => {
        selectedRows.includes(row.id) && (table += `<td>${row[header] ?? ''}</td>`);
      });
      table += '</tr>';
    });
        ...
        <div className="space-y-4">
          ...
          <div>
            ...
            <div className="grid lg:grid-cols-5 md-grid-cols-2 sm:grid-cols-1 gap-2 p-0">
              {['Check', 'DD', 'Bank Transfer', 'UPI', 'Card'].map((method, index) => (
  ...
  function AddFeesModel(props){
    const [selectedMode, setSelectedMode] = useState('Cash');
    const paymentModes = ['Cash', 'Cheque', 'DD', 'Bank Transfer', 'UPI', 'Card'];

    const [fees, setFees] = useState(350.00); // Assuming fees is 350
    const [date, setDate] = useState('4/10/2025');
    const [payingAmount, setPayingAmount] = useState(10000);
    const [availableCount, setAvailableCount] = useState(10); // Sample value
    const [value, setValue] = useState(100.00); // Sample value
    const [discount, setDiscount] = useState(120);
    const [fine, setFine] = useState(98);
    const [note, setNote] = useState('None');

    const exportExcel=()=>{
      const headers=["Fees","Date","Paying Amount","Available Count","Value","Discount","Fine","Note"];

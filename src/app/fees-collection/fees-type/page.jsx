...
export default function FeesType() {
  ...
  const filteredRoles = feesType.filter(role =>
  ...
  const handleDelete = (name) => {
    setFeesType(feesType.filter(role => role.name !== name));
  };

  const handleExcelExport = () => {
    let table = `<table border="1"><tr><th>Name</th><th>Fees Code</th></tr>`;
    feesType.forEach(role => {
        ...
        <div className="bg-white p-4 sm:p-6 rounded-lg shadow">
          ...
          {showGrid && (
            <div className="flex flex-wrap gap-2 mb-4">
              <button className={` px-3 py-1 text-sm border rounded focus:border-2 ${displaySection[0] && 'btn-primary'}`} onClick={() => {
                const updated = [...displaySection];
                updated[0] = !updated[0];
                setDisplaySection(updated);
              }}>Name</button>
              <button className={` px-3 py-1 text-sm border rounded focus:border-2 ${displaySection[1] && 'btn-primary'}`} onClick={() => {
                const updated = [...displaySection];
                updated[1] = !updated[1];
                setDisplaySection(updated);
              }}>Fees Code</button>
              <button className={`px-3 py-1 text-sm border rounded focus:border-2 ${displaySection[2] && 'btn-primary'}`} onClick={() => {
                const updated = [...displaySection];
                updated[2] = !updated[2];
                setDisplaySection(updated);
              }}>Action</button>

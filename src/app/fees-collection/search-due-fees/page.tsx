...
export default function FeeManagement() {
  ...
  const [displayedStudents, setDisplayedStudents] = useState<Student[]>(
    student.map(s => ({
      ...s,
      admission_no: typeof s.admission_no === 'string' ? Number(s.admission_no) : s.admission_no
    }))
  );

  const feesGroups: FeesGroup[] = [
  ...
  const toggleFeesGroup = (id: string): void => {
    if (id === 'all') {
  ...
  useEffect(() => {
    ...
    if (!searchQuery.trim()) {
      setDisplayedStudents(student.map(s => ({
        ...s,
        admission_no: typeof s.admission_no === 'string' ? Number(s.admission_no) : s.admission_no
      })));
      return;

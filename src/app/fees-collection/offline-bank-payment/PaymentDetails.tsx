...
type PaymentStatus = 'Approve' | 'Reject';

export default function PaymentDetailsModal({ payment, onClose }: PaymentDetailsModalProps) {
  const [status, setStatus] = useState<PaymentStatus>('Approve');
  const [amount, setAmount] = useState<string | number>(payment?.amount || 200.00);
  const [fine, setFine] = useState<string | number>(0);

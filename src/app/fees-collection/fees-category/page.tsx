...
const FeeCategoryManager: React.FC = () => {
  ...
  const [showCastArr, setShowCastArr] = useState<string[]>([]);
  const [showCastSection, setShowCasteSection] = useState<boolean>(false);
    ...
    <div className="lg:flex md:block gap-4 p-4">
      ...
      <div className="lg:w-1/3 md:w-full bg-white rounded shadow p-4">
        ...
        <div className="mb-3">
          ...
          />
          {save && name === "" && (
            <p className="text-red-600">Fill this Name<span>*</span></p>
          )}
        </div>
        ...
        <div className="mb-4">
          ...
          <div className="w-full flex justify-between items-center border rounded px-3 py-2 bg-white shadow-sm">
              ...
              {showCastArr.map((caste, index) => (
                ...
                >
                  ...
                  <button
                    ...
                    onClick={() =>
                      setShowCastArr(showCastArr.filter((_, i) => i !== index))
                    }
                  >
                    <X size={12} />
                  </button>
            ...
            <div
              onClick={() => setShowCasteSection(!showCastSection)}
            ...
            <div className="w-full border mt-1 rounded bg-white shadow-md z-10">
              {castType.map((castType, index) => (
                <div
                  key={index}
                  onClick={() => {
                    if (!showCastArr.includes(castType))
                      setShowCastArr([...showCastArr, castType]);
                  }}
                  className={
                    !showCastArr.includes(castType)
                      ? "w-full py-2 px-4 hover:bg-gray-100 cursor-pointer text-sm"
                      : ""
                  }
                >

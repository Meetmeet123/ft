...
export default function FeesDiscount() {
  ...
  const [useNumberCount, setUseNumberCount] = useState<string>("");
  const [save, setSave] = useState<boolean>(false);

  const [showContent, setShowContent] = useState<boolean[]>(Array(7).fill(true));
  ...
  const handleExportExcel = (): void => {
    ...
    const headers = Object.keys(discounts[0]) as (keyof Discount)[];
    let table = '<table><tr>';
  ...
  const handleDiscountTypeChange = (type: 'fixed' | 'percentage'): void => {
    setDiscountType(type);
  };

  const handleShowContentToggle = (index: number): void => {
    const temp = [...showContent];
    temp[index] = !temp[index];
    setShowContent(temp);
  };

  const handleDeleteDiscount = (idx: number): void => {
    ...
    <div className="p-4 max-w-screen-xl mx-auto">
      <div className="lg:flex md:block w-full gap-6">
        ...
        <div className="bg-white lg:w-1/3 rounded shadow p-6">
          ...
          <div className="mb-4">
            ...
            <input 
              ...
              className="input w-full border p-2 rounded focus:outline-none focus:ring-2 focus:ring-blue-500 focus:bg-blue-600 focus:text-white" 
            />
          ...
          <div className="grid grid-cols-2 gap-4 mb-4">
            <div>
              ...
              <input 
                type="text" 
                value={useNumberCount}
                onChange={(e) => setUseNumberCount(e.target.value)}
                className="input w-full border p-2 rounded focus:outline-none focus:ring-2 focus:ring-blue-500 focus:bg-blue-600 focus:text-white" 
              />
          ...
          <div className="mb-4">
            ...
            <textarea 
              rows={3} 
              className="input w-full border p-2 rounded resize-none focus:outline-none focus:ring-2 focus:ring-blue-500 focus:bg-blue-600 focus:text-white"
            ></textarea>
          </div>
        ...
        <div className="bg-white rounded shadow p-6 ">
          ...
          {showGrid && (
            <div className='relative flex justify-end w-full items-center gap-4 m-4'>
              {["Name", "Discount Code", "Percentage", "Amount", "No. of Count", "Expire", "Action"].map((btnName, index) => (
                <button 
                  key={index} 
                  onClick={() => handleShowContentToggle(index)}
                  className={`p-3 rounded focus:outline-none focus:ring-none ${showContent[index] && 'btn-primary'} `} 
                >
                  {btnName}
                </button>
              ))}

...
interface ComplainTypeFormProps {
  editData?: {
    id: number;
...
const ComplainTypeForm = ({ editData, refreshList }: ComplainTypeFormProps) => {
  ...
  useEffect(() => {
    if (editData) {
      setFormData({
        complaint_type: editData.complaint_type,
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const url = editData
        ? `https://school2025.dolittletech.co.in/api/complaint-types/${editData.id}`
      ...
      message.success(
        editData 
          ? "Complain type updated successfully" 
          : "Complain type created successfully"
      );
...
export default ComplainTypeForm;
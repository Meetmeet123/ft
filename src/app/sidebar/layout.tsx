  ...
  const menuItems: MenuItem[] = [
    ...
    },
    {
      ...
      subItems: [
        ...
        { title: "Add Homework", path: "/homework", icon: "plus-circle" },
  ...
  const renderMenuItem = (item: MenuItem, level: number = 0) => {
    ...
    return (
      <li key={item.title}>
        <Link
          href={item.path || "#"}
          className={`flex items-center p-3 text-slate-700 hover:bg-slate-200/50 transition-colors duration-200 ${isActive ? "bg-[#e0f7fa] text-[#1e40af] font-medium" : ""
            } ${level > 0 ? "pl-8" : ""}`}
          onClick={(e) => {
            if (hasSubItems) {
              e.preventDefault();
        ...
        >
          ...
          <div className="flex-1 flex items-center justify-between font-medium">
            ...
            {hasSubItems && (
              <div
                className={`w-4 h-4 flex items-center justify-center transition-transform duration-200 ${isOpen ? "rotate-180" : "rotate-0"
                  }`}
              >
                <ChevronDown size={16} />
              </div>

...
function MultiClassStudent() {
  ...
  const handleSearch = () => {
    ...
    const selectedClass = classMap[classValue];
    if (selectedClass) {
      const studentsList = selectedClass[sectionIndex] || [];
      const formattedStudents = studentsList.map((name) => ({
        name,
  ...
  const removeAssignment = (studentIndex, assignmentIndex) => {
    setStudentData((prev) =>
      prev.map((student, i) =>
        i === studentIndex
          ? {
        ...
        <div className="grid lg:grid-cols-2 md:grid-cols-1 gap-6">
          {studentData.map((student, studentIndex) => (
            <div key={studentIndex} className="border min-h-60 rounded shadow p-4">
              ...
              {student.assignments.map((assign, assignIndex) => (

      ...
      {discounts.map((discount) => (
        <div key={discount.id} className="border-b">
          ...
          <div className="flex items-center px-5 py-3 border-b bg-gray-50">
            <button 
              onClick={() => toggleSection(discount.id)}
              className="mr-2"
              aria-expanded={expandedSections[discount.id]}
              aria-controls={`${discount.id}-content`}
              aria-label={`Toggle ${discount.displayName} details`}
              type="button"
            >

...
import data2Raw from "./Class2LumpSum";

// Ensure "Amount (₹)" is a string in data2
const data2 = Array.isArray(data2Raw)
  ? data2Raw.map((item) => ({
      ...item,
      ["Amount (₹)"]: String(item["Amount (₹)"]),
    }))
  : { ...data2Raw, ["Amount (₹)"]: String(data2Raw["Amount (₹)"]) };
...
import data8Raw from "./Class4General";

// Ensure "Amount (₹)" is a string in data8
const data8 = Array.isArray(data8Raw)
  ? data8Raw.map((item) => ({
      ...item,
      ["Amount (₹)"]: String(item["Amount (₹)"]),
    }))
...
export default function FeesDetails() {
  ...
  return (
    <div className="container mx-auto p-4">
      ...
      <div className="flex md:grid-cols-2 gap-4 mb-6">
        <div className="w-1/2">
          <label
            htmlFor="hostel"
          ...
          <div key={fee.id}>
            <div className="border-b last:border-b-0 flex items-center p-3 hover:bg-gray-50">
              ...
              <input
                ...
                id={`fee-${fee.id}`}
              />
              <label htmlFor={`fee-${fee.id}`} className="flex-grow">
                {fee.name}
              </label>

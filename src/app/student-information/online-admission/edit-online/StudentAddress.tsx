...
const StudentAddress: React.FC = () => {
  // State for checkboxes and radio buttons
  const [isGuardianAddressCurrent, setIsGuardianAddressCurrent] = useState(false);
  const [isPermanentAddressCurrent, setIsPermanentAddressCurrent] = useState(false);
  const [rte, setRte] = useState<"yes" | "no" | "">("");

  // Handlers
  const handleGuardianAddressChange = (e: ChangeEvent<HTMLInputElement>) => {
    setIsGuardianAddressCurrent(e.target.checked);
  };

  const handlePermanentAddressChange = (e: ChangeEvent<HTMLInputElement>) => {
    setIsPermanentAddressCurrent(e.target.checked);
  };

  const handleRteChange = (e: ChangeEvent<HTMLInputElement>) => {
    setRte(e.target.value as "yes" | "no");
  };
                ...
                <div className="mb-4">
                  <label className="inline-flex items-center">
                    <input
                      ...
                      checked={isGuardianAddressCurrent}
                ...
                <div className="mb-4">
                  <label className="inline-flex items-center">
                    <input
                      type="checkbox"
                      className="form-checkbox h-5 w-5 text-blue-600"
                      checked={isPermanentAddressCurrent}

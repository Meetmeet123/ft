...
import rawData1 from "./Class2General";
import rawData2 from "./Class2LumpSum";
import rawData3 from "./Ayan";
import rawData4 from "./Class5General";
import rawData5 from "./Class1General";
import rawData6 from "./Fees";

// Convert "Amount (₹)" to string for each item in data arrays
const data1 = Array.isArray(rawData1)
  ? rawData1.map((item) => ({
      ...item,
...
const data2 = Array.isArray(rawData2)
  ? rawData2.map((item) => ({
      ...item,
      ["Amount (₹)"]: String(item["Amount (₹)"]),
    }))
...
const data3 = Array.isArray(rawData3)
  ? rawData3.map((item) => ({
      ...item,
      ["Amount (₹)"]: String(item["Amount (₹)"]),
    }))
...
const data4 = Array.isArray(rawData4)
  ? rawData4.map((item) => ({
      ...item,
      ["Amount (₹)"]: String(item["Amount (₹)"]),
    }))
...
const data5 = Array.isArray(rawData5)
  ? rawData5.map((item) => ({
      ...item,
      ["Amount (₹)"]: String(item["Amount (₹)"]),
    }))
...
const data6 = Array.isArray(rawData6)
  ? rawData6.map((item) => ({
      ...item,
      ["Amount (₹)"]: String(item["Amount (₹)"]),
    }))
...
import rawData7 from "./exam";

const data7 = Array.isArray(rawData7)
  ? rawData7.map((item) => ({
      ...item,
      ["Amount (₹)"]: String(item["Amount (₹)"]),
    }))
...
import rawData8 from "./Class4General";

const data8 = Array.isArray(rawData8)
  ? rawData8.map((item) => ({
      ...item,
      ["Amount (₹)"]: String(item["Amount (₹)"]),
    }))
...
import rawData9 from "./Class3General";

const data9 = Array.isArray(rawData9)
  ? rawData9.map((item) => ({
      ...item,
      ["Amount (₹)"]: String(item["Amount (₹)"]),
    }))
...
interface FeeDetail {
  ...
  name: string;
  amount: string;
  content: FeeContentItem[] | FeeContentItem;
  display: boolean;
}
          ...
          <div key={fee.id}>
            <div className="border-b last:border-b-0 flex items-center p-3 hover:bg-gray-50">
              ...
              <input
                ...
                id={`fee-${fee.id}`}
              />
              <label htmlFor={`fee-${fee.id}`} className="flex-grow cursor-pointer">
                {fee.name}
              </label>

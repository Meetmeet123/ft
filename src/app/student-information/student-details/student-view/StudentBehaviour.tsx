// pages/student/[id]/behaviour.tsx
'use client';
...
export default function StudentBehaviour() {
  ...
  const [behaviourData, setBehaviourData] = useState<BehaviourEntry[]>([
    {
      id: 1,
  ...
  const handleEdit = () => {
    const idStr = prompt('Enter ID of row to edit:');
    if (!idStr) return;

    const id = parseInt(idStr);
    const target = behaviourData.find((row) => row.id === id);
    if (!target) {
      alert('ID not found.');
      return;
    }
